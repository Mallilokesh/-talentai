  ...
  try {
    ...
    if (type === 'job-matches') {
      ...
      result = await ai(
        ...
        'You are an AI job matcher. Return valid JSON arrays only, no markdown.'
      )
    ...
    if (type === 'candidates') {
      ...
      result = await ai(
        ...
        'You are an AI talent sourcer. Return valid JSON arrays only, no markdown.'
      )
      const clean = result.replace(/```json|```/g, '').trim()
      return NextResponse.json({ data: JSON.parse(clean) })
    }
    ...
    console.error(err)
    return NextResponse.json({ error: 'AI request failed. Check your ANTHROPIC_API_KEY.' }, { status: 500 })
  }
}
